...
class PostService(
    ...
    fun getPosts(pageable: Pageable): Page<PostResponse> {
        ...
        if(isLogin()) {
            ...
            return posts.map { PostResponse.of(it, bookmarked[it.postId] == true, bookmarkCounts[it.postId] ?: 0L) }
        } else {
            return posts.map { PostResponse.of(it, false, bookmarkCounts[it.postId] ?: 0L) }
        }

    }

    fun searchPosts(search: String, pageable: Pageable): List<PostResponse> {
        return postRepository.findBySearch(search, pageable).map { PostResponse.of(it, false, 0) }
    }
}